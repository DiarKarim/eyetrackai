...
def calc_angles(x, y):
    # Is the center treated as the origin of the coordinate system, or should it start from the bottom left of the image?

    # If treating the bottom left as the origin?



    xAngle = (x / camera_width) * 180
    yAngle = (y / camera_height) * 180

    # If treating the center of the image as the origin:

    # # Center of coordinate system
    # center_x = camera_width / 2
    # center_y = camera_height / 2

    # # Mapping relative distances to servo angles (0-180)
    # x_angle = (x - center_x / camera_width) * 180
    # y_angle = (y - center_y / camera_height) * 180

    # return xAngle, yAngle
    return xAngle, 0 # Should only moved horizontally?
        ...
        while(cap.isOpened()):
            ...
            if ret:
                ...
                if circle:
                    ...
                    # Calculate the angles needed for the motors to move to
                    xAngle, yAngle = calc_angles(circle[0], circle[1])

                    # Send data to serial port
                    data_string = f"{xAngle},{yAngle}\n"
...
df.to_csv("result" + str(time.time())[-1-3:-1] + ".csv")
cap.release()
out.release()
